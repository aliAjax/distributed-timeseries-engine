...
var ErrQueryRead = errors.New("query series read failed")
...
func (e ReadFailure) Error() string { return fmt.Sprintf("read query %q: %v", e.Metric, e.Err) }
func (e ReadFailure) Unwrap() error { return nil }

func readFailure(metric string, err error) error {
	return ReadFailure{Metric: metric, Err: err}
}
...
func (e Executor) Execute(ctx context.Context, p query_planner.Plan) (Result, error) {
	...
	if err != nil {
		return Result{}, readFailure(p.Expr.Metric, err)
	}
