...
func (e ParseFailure) Error() string { return fmt.Sprintf("parse %q: %v", e.Input, e.Err) }
func (e ParseFailure) Unwrap() error { return nil }

func invalidQuery(input string, err error) error {
	return ParseFailure{Input: input, Err: err}
}
...
func Parse(input string) (Expr, error) {
	...
	if input == "" {
		return Expr{}, invalidQuery(input, errors.New("empty query"))
	}
	e := Expr{Metric: input}
	if p := strings.Index(input, "["); p >= 0 {
		end := strings.Index(input[p:], "]")
		if end < 0 {
			return e, invalidQuery(input, errors.New("range selector"))
		}
		dur, er := time.ParseDuration(input[p+1 : p+end])
		if er != nil {
			return e, invalidQuery(input, er)
		}
		e.Range = dur
		e.Metric = input[:p]
	}
	if p := strings.Index(e.Metric, "{"); p >= 0 {
		...
		if end < p {
			return e, invalidQuery(input, errors.New("label selector"))
		}
	...
	if e.Metric == "" {
		return e, invalidQuery(input, errors.New("metric required"))
	}
	return e, nil
}
